...
def run(console_log_level: str):
    ...
    # Load .env from project root (DEEPSEEK_API_KEY etc.)
    try:
        from dotenv import load_dotenv
        env_path = Path(__file__).parent / ".env"
        if env_path.exists():
            load_dotenv(dotenv_path=str(env_path))
            logger.info(f"已加载项目根 .env: {env_path}")
        else:
    ...
    # === 快速失败守卫：确保 DEEPSEEK_API_KEY 已正确加载 ===
    try:
        ds_cfg = config.character_config.agent_config.llm_configs.deepseek_llm
        ds_key = getattr(ds_cfg, "llm_api_key", "") or ""
        # 有效 key：以 sk- 开头、纯 ASCII、无占位符
        invalid = (
            "${" in ds_key
            or "placeholder" in ds_key
            or "fill-me" in ds_key
            or not ds_key.startswith("sk-")
            or not ds_key.isascii()
        )
        if invalid:
            logger.critical(
                "DeepSeek API Key 无效。请到 platform.deepseek.com 获取后，"
                "填入项目根目录 .env 的 DEEPSEEK_API_KEY=sk-xxx 再重启。"
            )
            sys.exit(1)
    except Exception as e:
        logger.critical(f"检查 DeepSeek API Key 失败: {e}")
        sys.exit(1)
