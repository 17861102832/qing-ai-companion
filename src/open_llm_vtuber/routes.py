    ...
    async def get_live2d_folder_info():
        ...
        for entry in os.scandir(live2d_dir):
            if entry.is_dir():
                ...
                # model3.json 可能位于模型根目录，也可能在 runtime/ 子目录，
                # 且文件名可能是 <folder>.model3.json 或通用 model3.json（如官方 Miara 样例）
                model_candidates = [
                    ...
                    os.path.join(live2d_dir, folder_name, "runtime", "model3.json"),
                ]
                model3_file = next((c for c in model_candidates if os.path.isfile(c)), None)

                if model3_file:
                    model3_file = model3_file.replace("\\", "/")
                    # Find avatar file if it exists
                    avatar_file = None
                    avatar_base = os.path.dirname(model3_file)
                    for ext in supported_extensions:
                        for fold in (avatar_base, os.path.join(avatar_base, "..")):
                            avatar_path = os.path.join(fold, f"{folder_name}{ext}")
