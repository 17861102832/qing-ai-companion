...
from .qing_control import init_qing_control_routes
from .voice_clone import init_voice_routes
...
class WebSocketServer:
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        ...
        self.app.include_router(
            init_qing_control_routes(self.default_context_cache),
        )
        # 声音克隆 + 对话-动作同步路由
        for vr in init_voice_routes(self.default_context_cache):
            self.app.include_router(vr)
