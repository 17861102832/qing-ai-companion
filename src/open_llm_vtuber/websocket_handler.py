...
class WebSocketHandler:
    ...
    async def _handle_tap(
        ...
        """Handle tap/hit interaction from frontend (回显兜底, 供后续前端上报)"""
        area = data.get("area", "body")
        context = self.client_contexts.get(client_uid)
        if not context:
            logger.warning(f"Tap 事件无对应上下文, client_uid={client_uid}")
            return
        human_name = context.character_config.human_name or "用户"
        reply = f"（被{human_name}{('点了下头' if area == 'head' else '戳了下身体')}）诶？"
        await websocket.send_text(json.dumps({"type": "tap-echo", "reply": reply}))
        logger.info(f"Tap 回显: area={area}, reply={reply}")
